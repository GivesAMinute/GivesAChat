...
/* ---------------------------------------------------------
   ...
   The overlay is transparent by default, because that is what
   every compositor needs and OBS was previously providing for
   us by injecting its own CSS.

   Opened as a normal web page, transparent renders WHITE, which
   is useless for reading chat on the iPad or on the public
   pop-out. So a background is opt-in:

     ?bg=dark     the old #131313
     ?bg=black    fully opaque black
     ?bg=131313   any 3- or 6-digit hex, with or without the #

   Anything unrecognised is ignored rather than guessed at — a
   typo should leave the overlay usable in OBS, not paint it an
   arbitrary colour on a live stream.
--------------------------------------------------------- */
...
export function applyBackground() {
  ...
  try {
    raw = (new URLSearchParams(location.search).get("bg") || "")
      .toLowerCase()
      .trim();
  } catch {
    return;   // malformed query string — stay transparent
  }

  if (!raw) return;

  let color = NAMED_BACKGROUNDS[raw] || null;

  if (!color && /^#?[0-9a-f]{3}([0-9a-f]{3})?$/.test(raw)) {
    color = raw.startsWith("#") ? raw : `#${raw}`;
  }

  if (!color) {
    console.warn(`[Overlay] unrecognised ?bg=${raw} — staying transparent`);
    return;
  }
  ...
  console.log(`[Overlay] background: ${color}`);
}
